...
class CustmastController extends Controller {
    ...
    public function accessRules() {
        return array(
            ...
            array('allow',
                'actions' => array('create', 'update', 'delete', 'save', 'DataCustmast', 'DataCustaddr', 'Addbookcode', 'Delbookcode', 'Editbookcode', 'Getbookcode', 'Getbookcode2', 'Validcustaddr', 'Validatedata', 'TestSec','loadModelcustaddr'),
                'users' => array('@'),
            ),
            array('allow',
                'actions' => array('admin'),
    ...
    public function actionView($menucod,$locatcd) {
        ...
        if (isset($locatcd)) {
            $this->renderPartial('view', array('custmast' => $this->loadModel($locatcd),'menucod'=>$menucod));
        }
    }
	...
	public function actionUpdate($menucod,$locatcd) {
		...
		//$locatcd = $_POST['locatcd'];
        if (isset($locatcd)) {
            $this->renderPartial('update', array('custmast' => $this->loadModel($locatcd),'menucod'=>$menucod));
        }
    }
    ...
    public function actionDelete() {
        ...
        if (!empty($locatcd)) {//ตรวจสอบว่าได้ส่งค่ามาให้ลบหรือไม่
            //Custmast::model()->deleteByPk($locatcd);
            //echo $locatcd;
			Custmast::model()->deleteAllByAttributes(array('locatcd'=>$locatcd));
			//$this->renderPartial('index', array('menucod'=>$menucod));
		}
    }
    ...
    public function actionDataCustaddr($locatcd){
        // $criteria = new CDbCriteria();
		// $criteria->select = "bookcode";
        // $criteria->condition = "locatcd =:locatcd";
        // $criteria->params = array(':locatcd' => $locatcd);
        // $custaddr = Custaddr::model()->findAll($criteria);
		//$custaddr = Custaddr::model()->findAllbySql("select bookcode from custaddr where locatcd='$locatcd'");
		$sql = "select bookcode,bookdesc,balance,datebalance,'view' as status,bookcode as bookcode_chk from custaddr where locatcd='$locatcd' order by locatcd";
        $command = Yii::app()->db->createCommand($sql);
        $custaddr = $command->queryAll();
		if(!empty($custaddr)){
			Yii::app()->session['databooklc'] = $custaddr;
			Yii::app()->session['databooklc_chk'] = $custaddr;
		}
        echo CJSON::encode($custaddr);
		//print_r($custaddr);
    }
	...
	public function actionValidatedata(){
        ...
        if($frmTable=='custaddr'){
            $data1 = $this ->loadModelcustaddr($data2);
            //$data1 = $idno_custaddr ->idno;
        }
        //$custaddr->locatcd = $_POST['locatcd'];
    ...
    public function actionTestSec(){
        ...
        try{
            ...
            $sql = "delete from custaddr where bookcode ='T1-002'";
            $command = Yii::app()->db->createCommand($sql);
            $custaddr = $command->query();

            $sql = "INSERT INTO custaddr (locatcd, bookcode, bookdesc) VALUES ('TEST1', 'T1-002', 'T1-0022')";
            $command = Yii::app()->db->createCommand($sql);
            $custaddr = $command->query();
    ...
    public function actionSave() {
		...
		if($chkstatus == 'insert'){
			...
			$custmast = $this->loadModel($_POST['locatcd_id']);
		}
		$custmast->locatcd    = $_POST['locatcd'];
        ...
        try {
            if ($custmast->save()) {
                foreach ($databooklc_chk as $row){
					$chk = Custaddr::model()->findByAttributes(array('bookcode' => $row['bookcode_chk']));

                    //print_r($chk." vs ".$row['bookcode_chk']);

					if((!empty($chk)) && ($row['status'] === 'delete')){
						$custaddr = Custaddr::model()->findByAttributes(array('bookcode' => $row['bookcode_chk']));
						$custaddr->delete();
					}else if((!empty($chk)) && ($row['status'] === 'update')){
						$custaddr = Custaddr::model()->findByAttributes(array('bookcode' => $row['bookcode_chk']));
						$custaddr->locatcd = $_POST['locatcd'];
						$custaddr->bookcode = $row['bookcode'];
						$custaddr->bookdesc = $row['bookdesc'];
						$custaddr->balance = $row['balance'];
						$custaddr->datebalance = Yii::app()->CMyClass->convertDate($row['datebalance']);
						$custaddr->save();
					}else if((empty($chk)) && ($row['status'] === 'insert' || $row['status'] === 'update')) {
						$custaddr = new Custaddr();
						$custaddr->locatcd = $_POST['locatcd'];
						$custaddr->bookcode = $row['bookcode'];
						$custaddr->bookdesc = $row['bookdesc'];
						$custaddr->balance = $row['balance'];
                ...
                unset(Yii::app()->session['databooklc']);
				unset(Yii::app()->session['databooklc_chk']);
                //$this->renderPartial('view', array('custmast' => $custmast->locatcd));
            }
        } catch (Exception $e) {
            $transaction->rollback();
        }
    ...
    public function actionAddbookcode() {
        ...
        if (!empty($bookcode)) {
            $chk = Custaddr::model()->findByAttributes(array('bookcode' => $bookcode));
            if (Yii::app()->session['databooklc'] == null) {
                $databooklc = array();
                $databooklc_chk = array();
            } else {
    ...
    public function loadModel($locatcd) {
        $model = Custmast::model()->findByPk($locatcd);
        if ($model === null)
    ...
    //โหลดข้อมูล custaddr
    public function loadModelcustaddr($bookcode) {
        //$model = Custaddr::model()->findByAttributes(array('bookcode' => $bookcode));
        $sql = "select idno from custaddr where bookcode='$bookcode'";
        //print_r($sql);
        $command = Yii::app()->db->createCommand($sql);
        $custaddr = $command->query();
        $custaddr->bindColumn(1, $idno);
        if($idno==''){
            $idno=0;
        }
        return $idno;
